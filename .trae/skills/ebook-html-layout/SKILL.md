...
- **Brand Naming Rule**: Always capitalize the word "Lark" (never use "lark" in lowercase for the product name) across all titles, body text, and footers.
- H1
  - font-size: `36px`
  - line-height: `42px`
  - letter-spacing: `-2%`
...
## Feishu Doc Import & Multi-page Viewer

For ingesting long Feishu documents directly into a paginated ebook format:

```bash
npm run feishu:render -- --url "<feishu_doc_url>"
```

**Viewer Generation**:
Whenever multiple pages are generated (e.g., during the Feishu document ingestion workflow), the rendering script automatically generates a `viewer.html` file alongside the output files.
- `viewer.html` stitches all generated single pages (`34-01.html`, `34-02.html`, etc.) into a continuous scrolling view with a dark background.
- It mimics the reading experience of a PDF viewer.
- **AI Instruction**: Always proactively use the command `open <path_to_viewer.html>` to present this viewer to the user whenever the Feishu multi-page workflow is executed, as this provides a superior user experience compared to the `index.html` link list.
